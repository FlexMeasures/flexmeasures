...
* Let forecast automations use their own timezone and catch up only the latest missed occurrence after downtime, with skipped daylight-saving times handled once and repeated wall-clock times not duplicated [see `PR #2396 <https://github.com/FlexMeasures/flexmeasures/pull/2396>`_]
...
* One-off reporting jobs can be triggered via the API (``POST /assets/(id)/reports/trigger``), mirroring how forecasts and schedules are triggered, while respecting access to every input and output sensor and keeping outputs within the requested asset's subtree [see `PR #2298 <https://www.github.com/FlexMeasures/flexmeasures/pull/2298>`_]
