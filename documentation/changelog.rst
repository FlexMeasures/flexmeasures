...
Bugfixes
-----------
* Annotation labels on sensor and asset charts no longer flicker while moving between beliefs, can be pinned by clicking their range, and use separate rows when pinned and hovered labels are visible together. Fast chart tooltips now show only value and time by default, plus the sensor on asset charts, with a *Full belief info* switch for the remaining fields [see `PR #XXXX <https://www.github.com/FlexMeasures/flexmeasures/pull/XXXX>`_]
