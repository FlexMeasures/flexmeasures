...
* The asset's *Automations* page now shows when each active automation is due to run next, as a clock time in the automation's own schedule timezone, so a recurrence no longer has to be read back from its cron string; the same time is available as ``next_run`` on the automations API endpoints, in UTC, and is null while an automation is inactive; it is the next scheduled clock time, so it excludes catch-up work still pending, and it follows the dispatcher's daylight-saving rules, taking the first fold of a repeated local time and the first valid minute after a skipped one; the create and edit forms now spell out the five cron fields and what the schedule timezone does, and each row's *Run now*, *Edit*, *Activate*/*Deactivate* and *Delete* buttons form one aligned group, which lines up down the table whichever label the toggle carries [see `PR #2294 <https://www.github.com/FlexMeasures/flexmeasures/pull/2294>`_]
