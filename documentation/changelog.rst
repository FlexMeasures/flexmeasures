...
* An ``AggregatorReporter`` can take the portfolio it aggregates, and the sensor it records the aggregate on, from the asset's flex-context, which already describes both for scheduling, so a site's total consumption or production is reported without restating which sensors it is made of [see `PR #2579 <https://www.github.com/FlexMeasures/flexmeasures/pull/2579>`_]
