...
* Breaking behaviour change: explicitly turning off constraint relaxation is now honoured: setting ``relax-soc-constraints`` or ``relax-site-capacity-constraints`` to false keeps the respective constraints hard, even while ``relax-constraints`` defaults to true (previously these specific flags had no effect on their own); also, explicitly set breach prices (including zero prices) are never overwritten with default breach prices anymore, and setting only one breach price of the ``soc-minima``/``soc-maxima`` (or site capacity) pair no longer silently keeps the other constraint of the pair hard [see `PR #2252 <https://www.github.com/FlexMeasures/flexmeasures/pull/2252>`_]
