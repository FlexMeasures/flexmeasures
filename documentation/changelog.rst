...
----------------------
* Drop the redundant single-column indexes on ``timed_belief``'s ``event_start`` and ``sensor_id``, each of which was fully covered by a composite index leading with the same column, and require ``timely-beliefs`` 4.2.0, which no longer declares them [see `PR #2378 <https://www.github.com/FlexMeasures/flexmeasures/pull/2378>`_]
* Pin the column order of the ``timed_belief`` primary key and reorder it to ``(sensor_id, source_id, event_start, belief_horizon, cumulative_probability)``, so that it leads with the column nearly every query filters on and so that a separate composite index on its first four columns becomes redundant (this migration drops such an index if present). The set of key columns is unchanged, so this is a pure reordering: no table rewrite, and uniqueness semantics are identical [see `PR #2378 <https://www.github.com/FlexMeasures/flexmeasures/pull/2378>`_]
