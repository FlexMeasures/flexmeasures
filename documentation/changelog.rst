...
* Sensor references now accept a ``default`` fallback quantity, e.g. ``{"sensor": 50, "default": "0 kWh"}``, filling the time slots for which the referenced sensor holds no value; also settable from the flex-model UI [see `PR #2267 <https://www.github.com/FlexMeasures/flexmeasures/pull/2267>`_]
