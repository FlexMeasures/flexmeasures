...
* Improve chart axis domain for event values not around zero, with a per-sub-chart ``include-zero`` option in ``sensors_to_show`` (default on) editable from the graph editor [see `PR #2244 <https://www.github.com/FlexMeasures/flexmeasures/pull/2244>`_]
