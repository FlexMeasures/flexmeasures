...
* The jobs tab of an asset's status page now also lists the jobs of the asset's sub-assets, naming the asset each job happened on, so a site asset shows what happened anywhere below it, which you can switch off per session [see `PR #2500 <https://www.github.com/FlexMeasures/flexmeasures/pull/2500>`_]
