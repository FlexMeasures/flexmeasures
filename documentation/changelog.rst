...
Bugfixes
-----------
* An installed plugin listed in ``FLEXMEASURES_PLUGINS`` by name is no longer shadowed by a folder of that name in the working directory (which is easily the case when starting FlexMeasures from the plugin's own repository), where the plugin appeared to load but its routes and CLI commands went missing; spell out a path (e.g. ``./my_plugin``) to load such a folder on purpose [see `PR #PRNUM <https://www.github.com/FlexMeasures/flexmeasures/pull/PRNUM>`_]
