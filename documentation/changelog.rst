...
* The API is now rate-limited, with a generous default limit on all endpoints and a stricter limit on triggering schedules and forecasts. Limits are configurable, and can be set per account through the ``rate_limits`` account attribute [see `PR #2302 <https://www.github.com/FlexMeasures/flexmeasures/pull/2302>`_]
