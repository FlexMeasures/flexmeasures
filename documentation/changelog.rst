...
* Improve chart axis domain for event values not around zero, with a per-plot ``include-zero`` option in ``sensors_to_show`` (default on) editable from the graph editor [see `PR #2244 <https://www.github.com/FlexMeasures/flexmeasures/pull/2244>`_]
...
Bugfixes
-----------
* Fix the assets page's column sorting (by ID, Name and Account), which failed to trigger because a floating sticky-header plugin intercepted clicks on the real, DataTables-bound header cells; also fixed sorting by Account to order by the account's name instead of its (arbitrary) id
