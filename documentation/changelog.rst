...
* Breaking behaviour change: the top-level flex-context's ``relax-constraints`` field now defaults to ``True`` (matching the default already used within each ``commodities`` entry), so constraint violations are softly penalized by default instead of being hard constraints, unless explicitly set to ``False``. That default does not extend to the device ``consumption-capacity`` and ``production-capacity``, which stay hard unless relaxation is asked for, either by setting the device breach prices yourself, or by passing ``relax-constraints`` explicitly or setting ``relax-capacity-constraints`` to have them filled in for you. A directional device capacity may state a physical impossibility (a heat pump that cannot produce) rather than an economic preference, and a default should not make that breachable at a price. Explicitly given device breach prices are also respected now, instead of being overwritten by the defaults [see `PR #2172 <https://www.github.com/FlexMeasures/flexmeasures/pull/2172>`_ and `PR #2398 <https://www.github.com/FlexMeasures/flexmeasures/pull/2398>`_]
