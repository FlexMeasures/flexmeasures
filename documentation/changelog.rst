...
* A forecaster that is told both where to start training and how much history to train on now trains on whichever of the two asks for less data, rather than silently training back to the start date; asking for neither, or for only one of them, still resolves to the same training window as before [see `PR #2482 <https://www.github.com/FlexMeasures/flexmeasures/pull/2482>`_]
