...
----------------------
* ``uv run poe clean-db`` now also works on macOS, where Homebrew and Postgres.app make the installing user a superuser instead of creating the ``postgres`` system account that the script used to rely on [see `PR #2406 <https://www.github.com/FlexMeasures/flexmeasures/pull/2406>`_]
* ``uv run poe clean-db`` now actually receives its ``--db-name`` and ``--db-user`` arguments, and can read your answers to its password and confirmation prompts; note that the arguments are passed as ``--db-name my-db``, not as ``db-name=my-db`` [see `PR #2406 <https://www.github.com/FlexMeasures/flexmeasures/pull/2406>`_]
