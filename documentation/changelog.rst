...
* Speed up container startup by installing FlexMeasures itself as a regular (non-editable) package in the Docker image, so its own source files are bytecode-compiled at build time like every other dependency, instead of being recompiled from scratch on every boot [see `PR #XXXX <https://www.github.com/FlexMeasures/flexmeasures/pull/XXXX>`_]
* Speed up container startup further by making the ``docker-compose.yml`` dev stack's ``gunicorn`` invocation preload the app in its master process (matching the Docker image's own ``CMD``), and by reusing the Alembic head-revision check the app already runs at boot instead of repeating its database round trip a second time [see `PR #XXXX <https://www.github.com/FlexMeasures/flexmeasures/pull/XXXX>`_]
* Speed up app boot by deferring the ``altair`` import (and the one-time chart-defaults setup that depends on it) until a chart is actually rendered [see `PR #XXXX <https://www.github.com/FlexMeasures/flexmeasures/pull/XXXX>`_]
