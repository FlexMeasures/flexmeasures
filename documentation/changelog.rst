...
* Flex-context commitments can be scoped to a subset of devices (via a new optional ``sensors`` list or ``group`` reference), binding their aggregate flow as one commitment instead of binding each device separately; a ``sensors`` scope can cherry-pick devices across electrical groups, while a ``group`` scope reuses a group's members (including its inflexible ones) [see `PR #2295 <https://www.github.com/FlexMeasures/flexmeasures/pull/2295>`_]
