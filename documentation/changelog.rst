...
* Both tabs of an asset's status page now name the asset each row belongs to, and the jobs tab can also list the jobs of the asset's sub-assets, so a site asset shows what happened anywhere below it, which you can switch on per session [see `PR #2500 <https://www.github.com/FlexMeasures/flexmeasures/pull/2500>`_]
