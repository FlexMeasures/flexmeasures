...
* Improve chart axis domain for event values not around zero, with a per-sub-chart ``fit-to-data`` option in ``sensors_to_show`` (default off) that fits a sub-chart's y-axis to its data instead of padding it out to zero, editable from the graph editor [see `PR #2244 <https://www.github.com/FlexMeasures/flexmeasures/pull/2244>`_]
