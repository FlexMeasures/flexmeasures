...
-------------------------
* Drop nine obsolete tables left over from before the ``GenericAsset``/``Sensor`` data model (``asset``, ``asset_type``, ``market``, ``market_type``, ``power``, ``price``, ``weather``, ``weather_sensor`` and ``weather_sensor_type``). Seven of them were already dropped in v0.18.0, but ``asset_type`` and ``weather_sensor_type`` were never dropped by any upgrade, and a database that was downgraded past that release and upgraded again kept all nine. You are asked to confirm if any of them still holds data [see `PR #XXXX <https://www.github.com/FlexMeasures/flexmeasures/pull/XXXX>`_]
