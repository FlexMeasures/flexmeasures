...
* On the asset context page, the flex-context editor no longer fails to load when a value in the asset's flex-context holds a double quote, such as a commitment's name, and the asset tree no longer fails to draw on unusual asset names [see `PR #2574 <https://www.github.com/FlexMeasures/flexmeasures/pull/2574>`_]
* Asset, sensor and account names, sensor units, graph titles and flex-model values are now shown as typed on the asset and sensor pages, where some characters in them could break the page [see `PR #2574 <https://www.github.com/FlexMeasures/flexmeasures/pull/2574>`_]
* In the asset tree, only the asset whose page it is is highlighted, where a sub-asset of the same name was highlighted too, and an asset named "Add asset" is no longer drawn as the button to add one [see `PR #2574 <https://www.github.com/FlexMeasures/flexmeasures/pull/2574>`_]
