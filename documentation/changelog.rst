...
Bugfixes
-----------
* Annotation searches can now be scoped by belief time: ``search_annotations`` on sensors, assets and accounts accepts ``beliefs_after``/``beliefs_before``, and both ``chart_annotations`` endpoints (sensor and asset) pass on ``prior`` (plus an undocumented ``beliefs_after``, for symmetry with the sibling ``chart``/``chart_data`` endpoints) instead of silently ignoring or not accepting them, so replaying a chart no longer shows annotations that were only recorded later; annotations without a belief time keep showing up regardless [see `PR #2367 <https://www.github.com/FlexMeasures/flexmeasures/pull/2367>`_]
* Continuing the kebab-case query-parameter migration started in PR #2352: the ``chart``, ``chart_data`` and ``chart_annotations`` query parameter previously named ``beliefs_before`` (on both the sensor and asset endpoints, in ``api/dev`` and ``api/v3_0``) is now named ``prior``, matching the name already used elsewhere in the API for the same concept (e.g. the scheduling trigger endpoint); their ``event_starts_after``/``event_ends_before`` fields are now ``start``/``end``, a new ``duration`` field lets either bound be derived from the other, and their remaining underscored fields are hyphenated. Old spellings keep working as legacy aliases [see `PR #2367 <https://www.github.com/FlexMeasures/flexmeasures/pull/2367>`_]
