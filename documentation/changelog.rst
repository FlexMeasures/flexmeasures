...
----------------------
* ``uv run poe clean-db`` now also works on macOS, where Homebrew and Postgres.app make the installing user a superuser instead of creating the ``postgres`` system account that the script used to rely on [see `PR #2408 <https://www.github.com/FlexMeasures/flexmeasures/pull/2408>`_]
* ``uv run poe clean-db`` now actually receives its ``--db-name`` and ``--db-user`` arguments, and can read your answers to its password and confirmation prompts; note that the arguments are passed as ``--db-name my-db``, not as ``db-name=my-db`` [see `PR #2408 <https://www.github.com/FlexMeasures/flexmeasures/pull/2408>`_]
* ``uv run poe clean-db`` now handles database and user names containing a dash, such as the documented ``flexmeasures-db``, whose privilege grants previously failed with a syntax error while the script still reported success; passwords containing a quote work as well [see `PR #2408 <https://www.github.com/FlexMeasures/flexmeasures/pull/2408>`_]
