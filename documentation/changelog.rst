...
.. warning:: Upgrading to this version requires to add SECURITY_TOTP_SECRETS (dictionary) to either the ``.env``, ``.flexmeasures.cfg`` or a new file named ``totp_secrets`` in the app's instance directory. FlexMeasures will not run without it. It will advise you how to set it on startup, however.
...
* Added :abbr:`2FA (two-factor authentication)` support for email and added ``_macros.html`` for UI components [see `PR #1555 <https://github.com/FlexMeasures/flexmeasures/pull/1555>`_]
