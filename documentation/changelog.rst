...
* A new automation can now be pointed at a data source that already holds a forecaster or a reporter, instead of naming that class and its configuration again, in the UI and the API as well as in the CLI. See `Automations, in detail`_ [see `PR #2584 <https://www.github.com/FlexMeasures/flexmeasures/pull/2584>`_]
...
* A forecast or report automation can now reuse an existing data source, in the API and the UI as well as in the CLI, so that several automations compute under one data generator, one configuration and one lineage of data. The *Automations* page searches the data sources you may read, by id, by name and by data generator class, and shows the configuration the picked one stores before the automation is created [see `PR #2584 <https://www.github.com/FlexMeasures/flexmeasures/pull/2584>`_]
