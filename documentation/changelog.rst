...
* Try out a forecast without recording it, using ``flexmeasures add forecasts --dry-run``, which computes the forecast in full and reports the sensor, data source, number of beliefs and event range it would have saved [see `PR #2494 <https://www.github.com/FlexMeasures/flexmeasures/pull/2494>`_]
...
* Speed up forecasting jobs by loading each sensor's data once per job, rather than once for every role the sensor plays in it, which most notably halves the data loading of an autoregressive forecast [see `PR #2481 <https://www.github.com/FlexMeasures/flexmeasures/pull/2481>`_]
...
* Which data source wins an event, when several report it and one value per event is asked for, now follows one rule: a source's version is only compared within a family of sources sharing a name, type and model, where it used to be compared between unrelated sources, so a forecaster at v9 no longer outranks a scheduler at v1; between families, the order in which a caller passed its sources decides, which is what the ``AggregatorReporter`` has documented all along; and a tie neither settles falls to the most recent belief and then the highest source id, rather than to the source whose name sorts first [see `PR #2494 <https://www.github.com/FlexMeasures/flexmeasures/pull/2494>`_]
