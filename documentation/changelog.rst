...
Bugfixes
-----------
* A sequential schedule no longer gets stuck when one of its devices fails to be scheduled by a scheduler without a fallback, which since the retirement of the storage fallback scheduler is every scheduler that does not define one itself: the failure now cascades to the remaining subjobs and to the wrap-up job whose id the trigger endpoint returns, so a client polling that job gets a terminal failure naming the device that could not be scheduled, instead of a job that stays deferred forever [see `PR #2409 <https://www.github.com/FlexMeasures/flexmeasures/pull/2409>`_]
