...
* Schedules as well as forecasts: a schedule automation stores what the schedule trigger endpoint accepts, and schedules from each run's own time, unless the trigger message fixes a ``start`` [see `PR #2293 <https://www.github.com/FlexMeasures/flexmeasures/pull/2293>`_]
...
* The *Automations* page now says how far off each next run is, such as "in 6 minutes" or "tomorrow", with the exact clock time, its timezone and its recurrence on hover; *Created At* reads on the automation's own clock too. The page lists what runs anywhere below the asset as well, naming the asset each automation belongs to, which you can switch off per session, and the API endpoint takes ``include_child_assets=false`` for the same. It keeps itself up to date about once a minute, holding off while the tab is in the background or a panel is open, so runs and job counts appear without reloading. The per-row *Details* button is now called *Info*, and its panel adds the configuration the data source was created with, such as the model a forecaster trains and over what period, which a new forecast or report automation can now be given on the *New automation* form as well as through the API and the CLI [see `PR #2554 <https://www.github.com/FlexMeasures/flexmeasures/pull/2554>`_]
