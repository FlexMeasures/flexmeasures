...
* ``flexmeasures add automation`` refuses a fixed ``start`` or ``prior`` in the trigger message of a schedule automation, and a fixed ``start``, ``end`` or ``belief_time`` in the parameters of a report automation, as every run would then cover the same period, or ignore the data recorded since. For a report, it points to ``start-offset`` and ``end-offset`` instead.
