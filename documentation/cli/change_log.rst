...
* ``flexmeasures add automation --type reporting`` refuses a fixed ``start`` or ``end`` in the report parameters, as every run would then report on the same period, and points to ``start-offset`` and ``end-offset`` instead.
