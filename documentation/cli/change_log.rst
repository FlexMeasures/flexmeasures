...
* ``flexmeasures add automation`` refuses a fixed ``start`` in the trigger message of a schedule automation, and a fixed ``start`` or ``end`` in the parameters of a report automation, as every run would then cover the same period. For a report, it points to ``start-offset`` and ``end-offset`` instead.
