...
.. note::
    ...
    The device ``consumption-capacity`` and ``production-capacity`` also stay hard unless you ask for them to be relaxed, either by setting ``consumption-breach-price`` or ``production-breach-price`` yourself, or by passing ``relax-constraints`` explicitly or setting ``relax-capacity-constraints`` to have those prices filled in for you.
