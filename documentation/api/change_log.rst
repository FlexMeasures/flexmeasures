...
- The ``chart``, ``chart_data`` and ``chart_annotations`` query parameter previously named ``beliefs_before`` (``GET /assets/<id>/chart``, ``GET /assets/<id>/chart_data``, ``GET /assets/<id>/chart_annotations``, and their sensor equivalents under ``api/dev``) is now named ``prior``, matching the name already used for the same concept elsewhere in the API (see :ref:`beliefs`). The legacy ``beliefs_before`` spelling keeps working. New clients should prefer ``prior``.
- Fixed: ``GET /assets/<id>/chart_annotations`` and ``GET /sensors/<id>/chart_annotations`` now also accept a ``beliefs_after`` query parameter, matching their sibling ``chart``/``chart_data`` endpoints; the asset endpoint previously accepted neither ``beliefs_after`` nor (a working) ``prior``/``beliefs_before``.
