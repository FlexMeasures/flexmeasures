...
- Breaking: the built-in storage fallback scheduler has been retired. ``GET /api/v3_0/sensors/<id>/schedules/<uuid>`` now returns ``400`` with the failure reason for an infeasible storage schedule instead of a ``303`` redirect to (or an automatic follow of) a fallback schedule. ``FLEXMEASURES_FALLBACK_REDIRECT`` remains relevant only for custom schedulers that still define a fallback scheduler. Setting ``relax-soc-constraints`` to ``false`` now keeps ``soc-minima``/``soc-maxima`` hard, and setting ``relax-site-capacity-constraints`` to ``false`` now keeps the site capacities hard; previously the ``relax-constraints`` default overrode them, so these fields had no effect on their own. Explicitly set breach prices (including zero prices) are never overwritten with default breach prices anymore, and when only one breach price of the ``soc-minima``/``soc-maxima`` (or site capacity) pair is set explicitly, the other one still gets its default breach price instead of silently becoming a hard constraint.
