...
- **Field canonicalization** for background job tracking:
  ...
  * Legacy ``schedule`` field (in scheduling endpoints) and ``forecast`` field (in forecasting endpoints) are deprecated but remain in responses for backward compatibility. Responses containing these fields include ``Deprecation`` and ``Link`` headers pointing to migration guidance.
...
- ``GET /api/v3_0/jobs/<uuid>`` now returns kebab-case metadata fields such as ``func-name`` and ``enqueued-at``. Legacy snake_case metadata fields such as ``func_name`` and ``enqueued_at`` remain available and are signalled with ``Deprecation`` and ``Link`` headers.
