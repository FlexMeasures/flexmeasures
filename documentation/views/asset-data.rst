...
For jobs, switch on "Include jobs of sub-assets" to also list those of the asset's sub-assets, so that a site asset shows what happened anywhere below it, even when jobs are triggered on a child asset.
Here too, an *Asset* column names the asset each job happened on, which is what tells the levels apart.
We remember that choice for the rest of your session.
