...
def log_error(exc: Exception, error_msg: str):
    ...
    extra = dict(
        user="a1 Test User",
        url=request.path,
        **get_err_source_info(last_traceback)
    )

    msg = "{error_name}:{message} [occured at {src_module}({src_func}):{src_linenr}, URL was: {url}, user was: {user}]"\
          .format(error_name=exc.__class__.__name__, message=error_msg, **extra)
...
def get_err_source_info(original_traceback=None) -> dict:
    ...
    try:  # carefully try to get the actual place where the error happened
        ...
        return dict(src_module=first_call[0],
                    src_linenr=first_call[1],
                    src_func=first_call[2],
                    src_code=first_call[3])
    except Exception as e:
        current_app.warning('I was unable to retrieve error source information: %s.' % str(e))
