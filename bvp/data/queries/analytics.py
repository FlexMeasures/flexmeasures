...
def get_power_data(showing_pure_consumption_data: bool, metrics: dict)\
    ...
    if not power_forecast_data.empty:
        ...
        metrics["mae_power_in_mwh"] = calculations.mean_absolute_error(realised_power_in_mwh,
                                                                       expected_power_in_mwh)
        metrics["mape_power"] = calculations.mean_absolute_percentage_error(realised_power_in_mwh,
                                                                            expected_power_in_mwh)
...
def get_prices_data(metrics: dict) -> Tuple[pd.DataFrame, Union[None, pd.DataFrame], dict]:
    ...
    if not prices_forecast_data.empty:
        ...
        metrics["mape_unit_price"] = calculations.mean_absolute_percentage_error(prices_data.y,
...
def get_revenues_costs_data(power_data: pd.DataFrame, prices_data: pd.DataFrame,
    ...
    if power_data.empty or prices_data.empty or power_forecast_data.empty or prices_forecast_data.empty:
