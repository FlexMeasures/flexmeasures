...
filename = "raw_data/VPP_DesignDataSetR02_(Jeju_GPS_180219).xls"
...
def extract() -> pd.DataFrame:
    ...
    df = pd.read_excel(
        ...
        skiprows=[0],
        header=None,
        names=["asset_type", "display_name", "id", "capacity", "location"],
    )
...
def update_asset(
    asset: dict, display_name: str, capacity: Optional[float], location: Tuple[str, str]
):
...
def update_assets(metadata: pd.DataFrame):
    ...
    with open("raw_data/assets.json", "r") as assets_json:
        assets = json.loads(assets_json.read())
        for asset_md_id in metadata.index:
            asset_md = metadata.loc[asset_md_id]
            for asset in assets:
                if asset["name"] == asset_md_id or asset["name"] == asset_md_id.replace(
                    "-", "_"
                ):
                    print(
                        "Found %s (%s - it's a %s asset)"
                        % (asset_md_id, asset_md.display_name, asset_md.asset_type)
                    )
                    update_asset(
                        asset,
                        asset_md.display_name,
                        asset_md.capacity,
                        asset_md.location,
                    )
                    break
                ...
                if asset_md.asset_type == "ChargingStation":
                    ...
                    candidates = [
                        a
                        for a in assets
                        if a["asset_type_name"] == "charging_station"
                        and a["name"] not in matched_charging_stations
                    ]
                    if candidates:
                        print(
                            "Putting %s's name and location to %s"
                            % (asset_md.display_name, candidates[0]["name"])
                        )
                        update_asset(
                            candidates[0],
                            asset_md.display_name,
    ...
    with open("raw_data/assets.json", "w") as assets_json:
