...
Guard FlexMeasures' domain model, invariants, and long-term architecture. Ensure PR changes respect domain boundaries, maintain model clarity, and prevent erosion of core architectural principles. This agent owns the integrity of assets, sensors, markets, scheduling, and their relationships.
...
- Scheduler implementations and flex_model/flex_context handling
- Domain boundaries between API, CLI, and internal services
...
- [ ] **Non-null flex_context**: Check that required flex_context fields are populated
- [ ] **Flex model validation**: Ensure flex_model conforms to `FlexModelSchema`
...
- **Key fields**: `id`, `name`, `account_id`, `parent_asset_id`, `attributes`, `flex_context`, `flex_model`
- **Relationships**: 
...
- **Purpose**: Measures events/timeseries on a GenericAsset
...
- **Path representation**: Asset > Asset > Sensor
...
- **Purpose**: Base class for optimization engines
- **Inputs**: 
  - Asset or Sensor
  - Time window: start, end, resolution, belief_time
  - flex_model + flex_context
- **Implementations**:
  - `StorageScheduler` - batteries, EVSEs, heat storage
...
4. **Timezone Awareness**
   ...
   - Naive datetimes assumed to be UTC (per `time_utils.py`)
