...
def test_a_stored_default_view_leaves_the_checkbox_alone(assert_js):
    assert_js(STUB_CHECKBOX + """
        ...
        eq("the view the user is on is what gets sent", JSON.parse(calls[0].options.body)["default-asset-view"], "Automations");
...
def test_a_refused_default_view_is_reported_and_unticked(assert_js):
    assert_js(STUB_CHECKBOX + """
        ...
        stubFetch(new Response(
            JSON.stringify({message: {json: {"default-asset-view": ["Must be one of: Context, Graphs."]}}}),
