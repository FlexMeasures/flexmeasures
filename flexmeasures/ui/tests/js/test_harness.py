...
def test_the_client_outlasts_the_page_and_the_wait_for_it():
    ...
    The page's own watchdog has to fire before the wait for it gives up,
    and both before the HTTP client does, or a busy machine produces a connection error
    naming nothing instead of a failed check naming what did not finish.
    """
...
def test_the_client_timeout_does_not_depend_on_the_global_socket_default():
    ...
    Any library that sets that global would otherwise decide how long our browser commands
    may take, which is how a busy machine came to fail with a read timeout.
    """
