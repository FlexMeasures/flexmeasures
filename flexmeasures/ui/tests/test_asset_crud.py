...
def _rendered_function(page: str, name: str) -> str:
    """Return the body of a JavaScript function as the page actually emits it.

    Jinja conditionals decide what reaches the browser,
    so the rendered page is what has to be checked, not the template.
    """
    ...
    raise AssertionError(f"the body of {name} is not closed in the rendered page")


def test_asset_kpis_are_asked_for_the_window_the_chart_shows(
    db, client, setup_ui_test_data, as_admin
):
    """getAssetKPIs must pass its window on untouched.

    Advancing the end date here was correct until PR #1909 gave this function its arguments,
    and re-advancing it would make every KPI cover a day the chart does not show.
    """
    # Built here rather than taken from a shared fixture,
    # since tests earlier in this module delete those assets.
    from flexmeasures.data.models.generic_assets import GenericAssetType

    admin = find_user_by_email("flexmeasures-admin@seita.nl")
    asset_type = (
    ...
    if asset_type is None:
        ...
        db.session.flush()
    asset = GenericAsset(
        name="kpi window rendering asset",
    ...
    db.session.flush()
    sensor = Sensor(
        name="kpi window rendering sensor",
        generic_asset=asset,
    ...
    response = client.get(
        url_for("AssetCrudUI:graphs", id=asset.id), follow_redirects=True
    )
    assert response.status_code == 200
    page = response.get_data(as_text=True)
    body = _rendered_function(page, "getAssetKPIs")

    assert "setDate" not in body, (
        "getAssetKPIs must not move the window it is given; "
        f"its callers already pass the chart's exclusive end. Rendered body:\n{body}"
    )
    assert (
        body.count("toIsoStringWithOffset") == 2
    ), "both ends of the KPI window should be formatted the same way"
