...
def keep_latest_version(
    ...
    Sources are grouped by (name, type, model, attributes_hash); within each group,
    only data from the latest version (breaking ties by highest id) is kept, per
    event or per belief. Probabilistic beliefs are kept intact. Sources that differ
    in their attributes (e.g. per-round community-scheduler sources) are distinct
    sources rather than versions of one another, so they are never deduplicated
    against each other (local rule, kept across the #2328 speedup).
    ...
    # Group unique sources by (name, type, model, attributes_hash)
    ...
    for i, source in enumerate(unique_sources):
        # Local rule (78c8c3ee6, kept across the #2328 speedup): sources that
        # differ in their attributes (e.g. per-round community-scheduler
        # sources) are DISTINCT sources per the DataSource uniqueness
        # constraint, not versions of one another, so attributes_hash is part
        # of the group identity and they are never deduplicated against each
        # other. With per-round sources this typically makes every source the
        # sole member of its group, hitting the early exit below.
        key = (source.name, source.type, source.model, source.attributes_hash)
