...
def device_scheduler(  # noqa C901
    ...
    # NB device-level commitments (flow and stock alike) are coupled to device
    # flows/stocks by grouped_commitment_equalities (via device_group_lookup):
    # commitments without a device_group column default to one group per device,
    # so device-scalar StockCommitments (e.g. soc-minima/-maxima breach
    # commitments) are covered as well. The former per-device rule
    # (device_stock_commitment_equalities) was superseded by that grouped path
    # and had survived here as dead code (defined, never registered as a
    # Constraint); it has been removed, matching main.
