...
def test_list_of_sensor_ids():
    schema = SensorsToShowSchema()
    input_value = [42, 43]
    expected_output = [{"title": None, "sensors": [42]}, {"title": None, "sensors": [43]}]
    assert schema.deserialize(input_value) == expected_output
...
def test_invalid_sensor_in_list():
    ...
    with pytest.raises(ValidationError, match="'sensors' value must be a list of integers."):
...
def test_invalid_sensor_dict_without_sensors_key():
    ...
    with pytest.raises(ValidationError, match="Dictionary must contain either 'sensor' or 'sensors' key."):
        schema.deserialize(input_value)


def test_mixed_valid_inputs():
    schema = SensorsToShowSchema()
    input_value = [{"title": "Test", "sensors": [1, 2]}, {"title": None, "sensors": [3, 4]}, 5]
    expected_output = [
...
def test_string_json_input():
    schema = SensorsToShowSchema()
    input_value = '[{"title": "Test", "sensors": [1, 2]}, {"title": "Test2", "sensors": [3]}]'
    expected_output = [
        {"title": "Test", "sensors": [1, 2]},
