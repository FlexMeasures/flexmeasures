...
def upgrade():
    ...
    if tables_with_data:
        confirmation = (
            input(
                f"The following tables still have data and will be dropped by this upgrade: {tables_with_data}. Are you sure you want to upgrade the database? (y/n): "
            )
            .strip()
            .lower()
        )
        if not confirmation == "y":
            raise (ValueError("Cancel upgrade"))
